...
fn main() {
    ...
    // Create auction account, owned by the auction program if it doesn't exist
    // let seed = [1u8; 32];
    ...
    match res {
        Ok(_) => {
            println!("Created auction account {}", oracle_account.pubkey())
        }
    ...
    let oracle_data =
        auction::try_from_slice(&data).expect("Failed to deserialize");

    println!("auction.owner: {:?}", oracle_data.owner);
    println!("auction.price: {:?}", oracle_data.price);
